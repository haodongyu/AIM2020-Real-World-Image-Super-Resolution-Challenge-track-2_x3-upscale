...
predicted_img_dir = '/home/lichenghua/Haodong_Yu/EDSR_xyhe/experiment/enhanced_images'
ground_truth_dir = '/data/yuhaodong/AIM/TestLRX3/TestLR'
...
for root, dirs, files in os.walk(predicted_img_dir):
    for name in files:
        if name[-3:] == 'png':
            ...
            print('111111111', sr.shape)
           # print('11111111111', in_sr_path.split('/')[-1].replace('x3_', 'LR'))

            in_hr_path = os.path.join(ground_truth_dir, in_sr_path.split('/')[-1].replace('x3_', 'LR'))
            hr = Image.open(in_hr_path)

            hr = np.array(hr, dtype=np.float32)
            #crop the border for x3 scale
            if not hr.shape[0]%3==0:
                hr = hr[:-(hr.shape[0]%3), :, :]
            if not hr.shape[1]%3==0:
                hr = hr[:, :-(hr.shape[1]%3), :]
